    ...
    <div class="portlet bordered light myfbody">
        <div class="portlet-title tabbable-line">
            <ul class="nav nav-tabs" style="float:left;">
                <?php if (is_array($module)) { $count=count($module);foreach ($module as $i=>$t) { ?>
                <li class="<?php if ($page==$i) { ?>active<?php } ?>">
                    <a href="#tab_<?php echo $i; ?>" data-toggle="tab" onclick="$('#dr_page').val('<?php echo $i; ?>')"> <i class="<?php echo dr_icon($t['icon']); ?>"></i> <?php echo $t['name']; ?> </a>
                </li>
                <?php } } ?>
            </ul>
        </div>
        <div class="portlet-body">
            <div class="tab-content">
                ...
                <?php if (is_array($module)) { $count=count($module);foreach ($module as $i=>$m) { ?>
                    ...
                    <div class="form-body">
                        ...
                        <div class="form-group dr_search">
                            ...
                            <div class="col-md-9">
                                ...
                                <span class="help-block"><?php echo dr_lang('开启之后访问栏目页会定向到搜索页面，使栏目模板无效'); ?></span>
                            </div>
                        </div>
                        <div class="form-group dr_search">
                            <label class="col-md-2 control-label"><?php echo dr_lang('关键词匹配字段'); ?></label>
                            <div class="col-md-9">
                                <input class="form-control input-xlarge" type="text" name="data[<?php echo $i; ?>][field]" value="<?php if ($m['setting']['search']['field']) {  echo $m['setting']['search']['field'];  } else { ?>title,keywords<?php } ?>" >
                                <span class="help-block"><?php echo dr_lang('默认关键词匹配字段是title,keywords，只能设置主表字段，以逗号分隔'); ?></span>
                            </div>
                        </div>
                        ...
                        <div class="form-group dr_search param_rule_<?php echo $i; ?>_0" style="display: none;">
                            ...
                            <div class="col-md-9">
                                ...
                                <span class="help-block"><?php echo dr_lang('字段映射是指伪静态时将搜索字段重新命名，例如keyword|k: 意思是把k作为keyword，多个字段映射回车符号分隔'); ?></span>
                            </div>
                        </div>
                        <div class="form-group dr_search param_rule_<?php echo $i; ?>_1" style="display: none;">
                            ...
                            <div class="col-md-9">
                                <?php $c=0;  if (is_array($m['search_field'])) { $count=count($m['search_field']);foreach ($m['search_field'] as $k1=>$f1) { ?>
                                <label><select name="data[<?php echo $i; ?>][param_join_field][<?php echo $c; ?>]" class="form-control">
                                    <option value="">-</option>
                                    <?php if (is_array($m['search_field'])) { $count=count($m['search_field']);foreach ($m['search_field'] as $k=>$f) { ?>
