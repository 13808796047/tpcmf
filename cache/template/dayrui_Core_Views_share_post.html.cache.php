<?php if ($fn_include = $this->_include("header.html")) include($fn_include);  if ($is_verify) {  } else { ?>
<script>
    <?php echo $auto_form_data_ajax; ?>
    function dr_syncat() {
        var width = '40%';
        var height = '60%';
        var title = '<i class="fa fa-refresh"></i> <?php echo dr_lang('同步到其他栏目'); ?>';
        var url = '<?php echo dr_url(MOD_DIR.'/home/syncat_edit'); ?>';
        layer.open({
            type: 2,
            title: title,
            shadeClose: true,
    ...
    function dr_ajax_time_submit() {
        ...
        layer.open({
            ...
            content: "<?php echo dr_url(APP_DIR.'/time/add'); ?>&is_ajax=1"
        });
    }
...
<form action="" class="form-horizontal" method="post" name="myform" id="myform">
                            ...
                            <div class="form-group <?php if (!$is_category_show) { ?>hide<?php } ?>">
                                ...
                                <div class="col-md-9">
                                    ...
                                    <?php } else { ?>
                                        ...
                                        <?php if (!$is_verify && $module[setting][sync_category]) {  if (!$id) { ?>
                                        <label style="margin-right:10px"><a href="javascript:;" onclick="dr_syncat()">[<?php echo dr_lang('同步发布到其他栏目'); ?>]</a></label>
                                        <label>
                                            <input id="dr_syncatid" name="sync_cat" type="hidden" value="" />
                                            <span id="dr_syncat_text" class="label label-success" style="display: none;"><b id="dr_syncat_num">0</b></span>
                                        </label>
                    ...
                    <div class="portlet-body">
                        <div class="form-body">
                            ...
                            <div class="form-group">
                                <label class="col-md-2 control-label"><?php echo dr_lang('分享到官微'); ?></label>
                                <div class="col-md-9">
                                    <input type="checkbox" name="sync_weibo" value="1" <?php if ($weibo['open']) { ?>checked<?php } ?> data-on-text="<?php echo dr_lang('开启'); ?>" data-off-text="<?php echo dr_lang('关闭'); ?>" data-on-color="success" data-off-color="danger" class="make-switch" data-size="small">
                                </div>
                            </div>
                            <?php }  if ($module['setting']['flag']) { ?>
                            <div class="form-group">
                                <label class="col-md-2 control-label"><?php echo dr_lang('推荐位设置'); ?></label>
                                <div class="col-md-9">
                                    <div class="mt-checkbox-list">
                                        <?php if (is_array($module['setting']['flag'])) { $count=count($module['setting']['flag']);foreach ($module['setting']['flag'] as $i=>$t) { ?>
                                        <label class="mt-checkbox mt-checkbox-outline">
                                            <input name="flag[]" type="checkbox" <?php if (@in_array($i, (array)$myflag)) { ?> checked="checked" <?php } ?> value="<?php echo $i; ?>"> <i class="<?php echo dr_icon($t['icon']); ?>"></i> <?php echo $t['name']; ?>
                                            <span></span>
                                        </label>
                                        <?php } } ?>
                                    </div>
                            ...
                            <div class="form-group">
                                ...
                                <div class="col-md-9">
                                    <div class="mt-radio-inline">
                                        <label class="mt-radio mt-radio-outline"><input type="radio" onclick="$('.dr_close_msg').hide()" name="verify[status]" value="1" checked /> <?php echo dr_lang('通过'); ?> <span></span></label>
                                        <label class="mt-radio mt-radio-outline"><input type="radio" onclick="$('.dr_close_msg').show()" name="verify[status]" value="0" /> <?php echo dr_lang('拒绝'); ?> <span></span></label>
                                    </div>
                                </div>
                            </div>
                            <div class="form-group dr_close_msg" style="display:none">
                                <label class="col-md-2 control-label"><?php echo dr_lang('拒绝理由'); ?></label>
                                <div class="col-md-9">
                                    <textarea id="dr_verify_msg" class="form-control" style="height:100px" name="verify[msg]"></textarea>
    ...
    <div class="portlet-body form myfooter">
        <div class="form-actions text-center">
            ...
            <label><button type="button" onclick="$('#dr_is_draft').val(0);dr_ajax_submit('<?php echo dr_now_url(); ?>', 'myform', '2000', '<?php echo $reply_url; ?>')" class="btn green"> <i class="fa fa-save"></i> <?php echo dr_lang('提交审核'); ?></button></label>
            <?php } else if ($is_post_time) { ?>
            <label><button type="button" onclick="$('#dr_is_draft').val(0);dr_ajax_submit('<?php echo dr_now_url(); ?>&posttime=<?php echo dr_date($posttime, 'Y-m-d H:i:s'); ?>', 'myform', '2000')" class="btn green"> <i class="fa fa-save"></i> <?php echo dr_lang('保存内容'); ?></button></label>
            <?php } else { ?>
